...
All lists used in this document are infinite and sorted. All lists of lists are
sorted on their heads. First, lets define a merge function (for infinite sorted
lists).
...
Note that evaluating the head of the result triggers the evaluation of both
...
performant. In the worst case, evaluating the $k$th element can require
$O\left(k\right)$ running time. To see why this is, let's look at what
structure results from evaluating \texttt{fmergeAllNaive} on a list. Suppose we
have the list

\texttt{xs = x1 : x2 : x3 : x4 : ...}


When we call \texttt{fmergeAllNaive xs}, the resulting structure looks
something like Figure \ref{fmergeallnaive}.

\begin{figure}
\caption{\label{fmergeallnaive}Structure of fmergeAllNaive}

\includegraphics[height=0.3\textheight]{fig1}
...
prefix requires at most $O(n\log k)$ steps as any element needs to
be compared with at most $\log_{2}k$ others to bubble to the top.
Now here's a more efficient \texttt{fmergeAll} implementation which
makes use of \texttt{fmergePrefix}.
...
must have such a factor). The primes are then just the list of all numbers
larger than $1$ excluding the composites. To avoid unbounded recursion,
the number $2$ must be explicitly given as a prime.
